...
function handleTouchStart(event) {
  // Check if two fingers are used for the touch
...
function handleTouchMove(event) {
  if (!isTwoFingerTouch) {
    // Prevent scrolling for single finger touch
    event.preventDefault();
    
  } else {
    // Handle the two-finger scroll as needed
    let deltaY = lastTouchY - event.touches[0].clientY;
    lastTouchY = event.touches[0].clientY;
    // Scroll the element manually or perform other actions
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Optional: If the content or size of the registration form can change, you may need to call matchHeights again.
    // For example, if the registration form can expand/collapse or if there are dynamic elements that may change its size.
    // window.addEventListener('resize', matchHeights); // To handle window resizing
});